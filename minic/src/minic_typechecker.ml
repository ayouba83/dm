...
let typecheck_program (prog: prog) =
  ...
  let rec type_expr fdef local_env param_env = function
    ...
    | Elm(i, t) -> 
      let ti = type_expr fdef local_env param_env i in
      if ti = Int
      then
        let tt = type_expr fdef local_env param_env (Get t) in
        match tt with
        |Tab(typ) -> typ
        | _ -> type_error (type_to_string tt) "type[]" "get_element of tab"
      else type_error (type_to_string ti) "Int" "indice of get_element of tab"
    | Len(tab) -> 
      let t = type_expr fdef local_env param_env (Get tab) in
      match t with
      | Tab(_) -> Int
      | _ -> type_error (type_to_string t) "type[]" "len(expression)"
  in
  ...
  let typecheck_function (fdef: fun_def) =
    ...
    *)
    ...
    let rec typecheck_instr = function
      ...
      | Insert(pos, e, tab) -> 
        let tpos = type_expr fdef local_env param_env pos in
        if tpos = Int
        then 
          let ttab = type_expr fdef local_env param_env (Get tab) in
          match ttab with
          | Tab(te_supp) -> 
            let te = type_expr fdef local_env param_env e in
            if te <> te_supp
            then type_error (type_to_string te) (type_to_string te_supp) "insert_element in tab"
          | _ -> type_error (type_to_string ttab) "type[]" "insert_element of tab"
        else type_error (type_to_string tpos) "Int" "indice of insert_element in tab"
  ...
  let pred (_, t, v) =
    ...
    | Some va ->
      ...
      if (tva = t) || (tva = Tab(Void))    (*pour le cas des tableaux, {} a le type qu'on veut donc on a déclaré qu'il avait le type Tab(Void), {} est un tableau de type int[] et bool[] et ... *)
      then ()
